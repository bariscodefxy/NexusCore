...
use NexusCore\Core\Networking\Packets;
...
class PacketReader {
	...
	public function registerPacketType($type, $serverbound = true, $clientbound = true) {
		if ($serverbound) {
			$this->ServerboundPackets[constant('NexusCore\Core\Networking\\' . $type . '::id')] = $type;
		}
		if ($clientbound) {
			$this->ClientboundPackets[constant('NexusCore\Core\Networking\\' . $type . '::id')] = $type;
		}
	}
	...
	public function readPacket($client, $serverbound = true) {
		...
		$construct = "NexusCore\\Core\\Networking\\" . $type;
